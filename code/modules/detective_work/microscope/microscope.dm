...
/obj/machinery/microscope/use_tool(obj/item/W, mob/living/user, list/click_params)
	if(sample)
		if (istype(W, /obj/item/evidencebag))
			...
			if(bag.stored_item)
				to_chat(user,  "<span class = 'warning'> \The [bag] already has \a [bag.stored_item] in it.")
				return TRUE
			if(sample.w_class > WEIGHT_CLASS_NORMAL)
				to_chat(user, "<span class = 'warning'>\The [src]'s [sample.name] is too big for \the [bag].")
				return TRUE
			user.visible_message(
				"<span class = 'notice'> \The [user] transfers \a [sample] from \the [src] to \a [bag].",
				"<span class = 'notice'> You transfer \a [sample] from \the [src] to \a [bag]."
			)
	...
	if (istype(W, /obj/item/evidencebag))
		...
		if (B.stored_item)
			to_chat(user, ("<span class = 'notice'> "You insert \the [B.stored_item] from \the [B] into the microscope."))
			B.stored_item.forceMove(src)
			sample = B.stored_item
			B.empty()
			update_icon()
		else
			to_chat(user,  "<span class = 'warning'>\The [B] is empty!"))
		return TRUE

	if(!user.unEquip(W, src))
		return TRUE
	to_chat(user, ("<span class = 'notice'> You insert \the [W] into the microscope."))
	sample = W
	update_icon()
	return TRUE
...
/obj/machinery/microscope/physical_attack_hand(mob/user)
	. = TRUE
	if(!sample)
		to_chat(user,  "<span class = 'warning'> The microscope has no sample to examine.")
		return

	to_chat(user, ("<span class = 'notice'> "The microscope whirrs as you examine \the [sample].")

	to_chat(user, "<span class = 'notice'> Printing findings now...")
	...
	if("gunshot_residue" in evidence)
		...
		if(LAZYLEN(evidence["gunshot_residue"]))
			...
			for(var/residue in evidence["gunshot_residue"])
				report.info += "["<span class = 'notice'> "[residue]")]<br><br>"
		else
	...
	if("fibers" in evidence)
		if(LAZYLEN(evidence["fibers"]))
			...
			for(var/fiber in evidence["fibers"])
				report.info += "(<span class = 'notice'> Most likely match for fibers: [fiber]")]<br><br>"
		else
	...
	if("prints" in evidence)
		...
		if(LAZYLEN(evidence["prints"]))
			...
			for(var/prints in evidence["prints"])
				report.info += "<span class = 'notice'> "Fingerprint string: "
...
/obj/machinery/microscope/proc/remove_sample(mob/living/remover)
	if(!istype(remover) || remover.incapacitated() || !Adjacent(remover))
		return
	if(!sample)
		to_chat(remover,  "<span class = 'warning'> \The [src] does not have a sample in it.")
		return
	to_chat(remover, "<span class = 'notice'> You remove \the [sample] from \the [src].")
